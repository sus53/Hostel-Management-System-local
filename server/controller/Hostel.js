...
export const AddHostel = async (req, res) => {
    ...
    const { _id, title, description, price, location, sex, imagepath1, imagepath2, imagepath3, email } = req.body
    try {
        ...
        if (_id) {
            ...
            if (hostel) {
                const updatedHostel = await Hostel.findByIdAndUpdate(
                    hostel._id,
                    { title, description, price, location, sex, imagepath1, imagepath2, imagepath3, email, latlng, floor },
                );
                return res.status(200).json({ message: 'hostel updated sucessfully', success: true, hostel: updatedHostel })
            }
        }


        const newHostel = new Hostel({
            title, description, price, location, sex, imagepath1, imagepath2, imagepath3, latlng, email, floor
        })
...
export const FilterHostel = async (req, res) => {
    ...
    try {
        ...
        const hostelIdsWithMatchingReviews = matchingReviews.map(review => review.hostel);

        const filteredHostels = rating ? await Hostel.find({ title: { $in: hostelIdsWithMatchingReviews }, ...query }) : await Hostel.find(query);
