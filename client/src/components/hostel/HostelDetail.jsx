...
import { selectBooking } from "../../redux/Index";
...
export const HostelDetail = () => {
  ...
  const [booking, setBooking] = useState({ hostel: hostel.title, email });
  ...
  const floorBookingHandler = (floorIndex, roomName) => {
    if (!booking.room?.[floorIndex]?.[roomName]?.isTrue) {
      setBooking({ ...booking, totalPrice: (booking.totalPrice == null || undefined) ? 0 + +hostel.floor[floorIndex][roomName].price : (booking.totalPrice + +hostel.floor[floorIndex][roomName].price), room: { ...booking.room, [floorIndex]: { ...booking.room?.[floorIndex], [roomName]: { price: hostel.floor[floorIndex][roomName].price, isTrue: true } } } })
    }
    else {
      const updatedBooking = booking.room;
      delete updatedBooking[floorIndex][roomName];
      if (Object.keys(updatedBooking[floorIndex]).length === 0) delete updatedBooking[floorIndex];
      setBooking({ ...booking, room: updatedBooking, totalPrice: booking.totalPrice - +hostel.floor[floorIndex][roomName].price });
            ...
            <div className="description-onclick">
              ...
              {hostel.floor && Object.keys(hostel.floor).map(floorIndex => (
                <>
                  ...
                  {hostel.floor[floorIndex] && Object.keys(hostel.floor[floorIndex]).map(roomName => (
                    <>
                      <button key={roomName} className={`btn shadow-sm me-3 w-25 my-2 ${booking?.room?.[floorIndex]?.[roomName]?.isTrue ? "btn-success" : "btn-light"} `} onClick={() => floorBookingHandler(floorIndex, roomName)} >
                        <div><span className="text-capitalize" >{roomName}</span> Seater</div>
                        <div>Rs. {hostel.floor[floorIndex][roomName].price}</div>
                      </button >
                    </>
                  ))}
                </>
              ))}
              <button className=" btn btn-booking btn-secondary " onClick={() => { bookNowHandler() }} disabled={booking.room ? Object.keys(booking.room).length === 0 ? true : false : true} >
                Book now!
              </button>
                ...
                <div className="booking-section" >
                  <div className="container">
                    ...
                    <div>
                      ...
                      <input type="number" value={booking.duration} onChange={(e) => setBooking({ ...booking, duration: e.target.value, totalPrice: tPrice * e.target.value })} />
                    </div>
                    <div>
                      <label>Starting Date</label>
                      <input type="date" value={booking.startingDate} onChange={(e) => setBooking({ ...booking, startingDate: e.target.value })} />
                    </div>
                    <div className="flex-column" >
                      <label >Details</label>
                      <div className="d-block " >
                        <span className="" >Hostel Title:</span> {booking.hostel}
                      </div>
                    </div>
                    <div className="d-flex flex-column">
                      <label>Room Description</label>
                      {booking.room && Object.keys(booking.room).map((floorIndex) => (
                        <>
                          <div>Floor {+floorIndex}</div>
                          {Object.keys(booking.room[floorIndex]).map((roomName) => (
                            <>
                              <div className="d-flex justify-content-between" >
                                <label className="text-capitalize " >{roomName} Seater : </label>
                                <div className="text-end">Rs.{booking.room[floorIndex][roomName].price}</div>
                              </div>
                            </>
                          ))}
                        </>
                    ...
                    <div className="justify-content-between  border-top border-dark" >
                      <label>Total Amount :</label>
                      <span className="text-end" >Rs.{booking.totalPrice}</span>
                    </div>
                    <button className="btn btn-primary w-100 mt-3" onClick={() => verifyPayment()} >Proceed</button>
                  </div>
                </div>
              }
            </div>
