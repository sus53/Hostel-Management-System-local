...
function ApproveHostel() {
    ...
    return (
        <div className='approve-hostel'>
                ...
                </tr>
                {
                    hostels.length > 0 && hostels.map((hostel, i) => (
                        <tr className='user-info' key={i} >
                            ...
                            <td>
                                <button className='btn btn-secondary m-2' onClick={() => { setHostel(hostel); setIsDetail(true) }}  >View</button>
                            </td>
                        </tr>
                    ))
                }
            ...
            {isDetail && <div className='hostel-detail-bg'>
                <div className='hostel-detail d-flex'>
                    <div >
                        <i className="fa-solid fa-xmark" onClick={() => setIsDetail(false)} ></i>
                        <div>
                        ...
                        <div>
                            ...
                            <span className='form-control' >{hostel.floor && Object.keys(hostel.floor).map(floorIndex => (
                                <>
                                    <div >Floor {+floorIndex}</div>
                                    <div className='d-flex' >
                                        {hostel.floor[floorIndex] && Object.keys(hostel.floor[floorIndex]).map(roomName => (
                                            <>
                                                <div key={roomName} className={`me-3 w-25 my-2 `} >
                        ...
                        <div>
                            ...
                            <span className='form-control' >{hostel.description} </span>
                        </div>
                    </div>
                    <div className='form-control' >
                        {(hostel.imagepath1 && hostel.imagepath2 && hostel.imagepath3 !== undefined) ?
                            <div className='image-container' >
                                ...
                                {hostel.imagepath2 !== "undefined" ? <>
                                    <div>Image 2</div>
                                    <img src={`http://localhost:5000/assets/${hostel.imagepath2}`} />
                                </> : ""}
                                {hostel.imagepath3 !== "undefined" ? <>
                                    <div>Image 3</div>
                                    <img src={`http://localhost:5000/assets/${hostel.imagepath3}`} />
                                </> : ""}
                            </div> : <div className='text-center m-auto' >"No Images selected in this hostel"</div>
                        }
                    </div>
                    <button className='btn btn-primary approve' onClick={() => verifyHostelHandler(hostel._id, "Approved")} >Approve</button>
                    <button className='btn btn-danger deny' onClick={() => verifyHostelHandler(hostel._id, "Denied")} >Deny</button>
                </div>
            </div>}
        </div>
    )
